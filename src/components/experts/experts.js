...
const AppExperts = () => {
  ...
  useEffect(() => {
    const fetchConsultants = async () => {
      try {
        const leftResponse = await axios.get('http://localhost:8000/consultants/getLeft');
        const rightResponse = await axios.get('http://localhost:8000/consultants/getRight');
  ...
  const sendDataToServer = async (data) => {
    try {
      const response = await axios.post('http://localhost:8000/telegram/experts', data, {
        headers: {
        ...
        <div className="experts-content__experts">
          {leftConsultant && (
            <div className="experts-content__experts-card experts-card__one">
              <img src={`http://localhost:8000/uploads/${leftConsultant.photo}`} alt={leftConsultant.name} />
          ...
          {rightConsultant && (
            <div className="experts-content__experts-card experts-card__two">
              <img src={`http://localhost:8000/uploads/${rightConsultant.photo}`} alt={rightConsultant.name} />
